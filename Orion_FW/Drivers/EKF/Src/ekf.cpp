...
bool EKF::bUpdate(const Matrix& Y, const Matrix& U, float dt)
{
    ...
    /* =============== Calculate the Jacobian matrix of f (i.e. F) =============== */
    /* F = d(f(..))/dx |x(k-1|k-1),u(k-1)                               ...{EKF_1} */
    if (!bCalcJacobianF(F, X_Est, U, dt)) {
        return false;
    }
    ...
    /* =========================== Prediction of x & P =========================== */
    /* x(k|k-1) = f[x(k-1|k-1), u(k-1)]                                 ...{EKF_2} */
    if (!bNonlinearUpdateX(X_Est, X_Est, U, dt)) {
    		printf("k = \n");
        return false;
    }
    ...
    if (!bCalcJacobianH(H, X_Est, U, dt)) {
        return false;
    }
