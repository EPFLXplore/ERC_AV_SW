...
void ModbusStart(modbusHandler_t * modH)
{
	...
	if (modH->xTypeHW == USART_HW || modH->xTypeHW ==  USART_HW_DMA )
	{
	      ...
	      if ((modH->EN_Port1 != NULL))
          {
              // return RS485 transceiver to transmit mode
	    	  HAL_GPIO_WritePin(modH->EN_Port1, modH->EN_Pin1, GPIO_PIN_RESET);
	    	  HAL_GPIO_WritePin(modH->EN_Port2, modH->EN_Pin2, GPIO_PIN_RESET);
          }
...
if(modH->xTypeHW != TCP_HW)
    ...
    modH->u8Buffer[ modH->u8BufferSize ] = ((u16crc >> 8) & 0xFF);
    modH->u8BufferSize++;
    ...
    if(modH->xTypeHW == USART_HW || modH->xTypeHW == USART_HW_DMA )
    {
    	...
    	if ((modH->EN_Port1 != NULL) )
        {
    		...
    		HAL_GPIO_WritePin(modH->EN_Port1, modH->EN_Pin1, GPIO_PIN_SET);
    		HAL_GPIO_WritePin(modH->EN_Port2, modH->EN_Pin2, GPIO_PIN_SET);
        }
